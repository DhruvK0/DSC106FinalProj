...
function axis(orient, scale2) {
  ...
  function axis2(context) {
    var values = tickValues == null ? scale2.ticks ? scale2.ticks.apply(scale2, tickArguments) : scale2.domain() : tickValues, format2 = tickFormat2 == null ? scale2.tickFormat ? scale2.tickFormat.apply(scale2, tickArguments) : identity_default2 : tickFormat2, spacing = Math.max(tickSizeInner, 0) + tickPadding, range3 = scale2.range(), range0 = +range3[0] + offset, range1 = +range3[range3.length - 1] + offset, position = (scale2.bandwidth ? center : number)(scale2.copy(), offset), selection2 = context.selection ? context.selection() : context, path2 = selection2.selectAll(".domain").data([null]), tick = selection2.selectAll(".tick").data(values, scale2).order(), tickExit = tick.exit(), tickEnter = tick.enter().append("g").attr("class", "tick"), line = tick.select("line"), text = tick.select("text");
    ...
    dx = k2 ? padAngle : tau2 / n;
    {
      ...
      for (const i of groupIndex) {
        ...
        if (directed) {
          ...
          groups2[i] = { index: i, startAngle: x0, endAngle: x4, value: groupSums[i] };
        } else {
...
var Path = class {
  ...
  arc(x4, y4, r, a0, a1, ccw) {
    ...
    if (r < 0)
      throw new Error(`negative radius: ${r}`);
    let dx = r * Math.cos(a0), dy = r * Math.sin(a0), x0 = x4 + dx, y0 = y4 + dy, cw = 1 ^ ccw, da2 = ccw ? a0 - a1 : a1 - a0;
    if (this._x1 === null) {
...
function contours_default() {
  ...
  contours.thresholds = function(_) {
    return arguments.length ? (threshold2 = typeof _ === "function" ? _ : Array.isArray(_) ? constant_default6(slice2.call(_)) : constant_default6(_), contours) : threshold2;
  };
...
function density_default() {
  ...
  density.thresholds = function(_) {
    return arguments.length ? (threshold2 = typeof _ === "function" ? _ : Array.isArray(_) ? constant_default6(slice2.call(_)) : constant_default6(_), density) : threshold2;
  };
...
var Path2 = class {
  ...
  arc(x4, y4, r) {
    ...
    if (this._x1 === null)
      this._ += `M${x0},${y0}`;
    else if (Math.abs(this._x1 - x0) > epsilon6 || Math.abs(this._y1 - y0) > epsilon6)
      this._ += "L" + x0 + "," + y0;
    if (!r)
      return;
    this._ += `A${r},${r},0,1,1,${x4 - r},${y4}A${r},${r},0,1,1,${this._x1 = x0},${this._y1 = y0}`;
  }
  rect(x4, y4, w, h) {
  ...
  *neighbors(i) {
  ...
  _clipSegment(x0, y0, x1, y1, c0, c1) {
    ...
    while (true) {
      ...
      let x4, y4, c6 = c0 || c1;
      if (c6 & 8)
        x4 = x0 + (x1 - x0) * (this.ymax - y0) / (y1 - y0), y4 = this.ymax;
      else if (c6 & 4)
        x4 = x0 + (x1 - x0) * (this.ymin - y0) / (y1 - y0), y4 = this.ymin;
      else if (c6 & 2)
        y4 = y0 + (y1 - y0) * (this.xmax - x0) / (x1 - x0), x4 = this.xmax;
      else
        y4 = y0 + (y1 - y0) * (this.xmin - x0) / (x1 - x0), x4 = this.xmin;
      if (c0)
        x0 = x4, y0 = y4, c0 = this._regioncode(x0, y0);
      else
        x1 = x4, y1 = y4, c1 = this._regioncode(x1, y1);
    }
  }
  ...
  _project(x0, y0, vx, vy) {
    ...
    if (vy < 0) {
      ...
      if ((c6 = (this.ymin - y0) / vy) < t)
        y4 = this.ymin, x4 = x0 + (t = c6) * vx;
    } else if (vy > 0) {
      if (y0 >= this.ymax)
        return null;
      if ((c6 = (this.ymax - y0) / vy) < t)
        y4 = this.ymax, x4 = x0 + (t = c6) * vx;
    }
    if (vx > 0) {
      if (x0 >= this.xmax)
        return null;
      if ((c6 = (this.xmax - x0) / vx) < t)
        x4 = this.xmax, y4 = y0 + (t = c6) * vy;
    } else if (vx < 0) {
      if (x0 <= this.xmin)
        return null;
      if ((c6 = (this.xmin - x0) / vx) < t)
        x4 = this.xmin, y4 = y0 + (t = c6) * vy;
    }
    return [x4, y4];
  }
  _edgecode(x4, y4) {
...
function autoType(object) {
  for (var key in object) {
    var value = object[key].trim(), number5, m3;
    if (!value)
      value = null;
    else if (value === "true")
      value = true;
...
function link_default(links) {
  var id2 = index2, strength = defaultStrength, strengths, distance = constant_default7(30), distances, nodes, count3, bias, random, iterations = 1;
  if (links == null)
    links = [];
  ...
  function initialize() {
    if (!nodes)
      return;
    var i, n = nodes.length, m3 = links.length, nodeById = new Map(nodes.map((d, i2) => [id2(d, i2, nodes), d])), link2;
...
function cluster_default() {
  ...
  function cluster(root3) {
    ...
    var left2 = leafLeft(root3), right2 = leafRight(root3), x0 = left2.x - separation(left2, right2) / 2, x1 = right2.x + separation(right2, left2) / 2;
...
function encloseBasis2(a4, b) {
  var x1 = a4.x, y1 = a4.y, r1 = a4.r, x22 = b.x, y22 = b.y, r2 = b.r, x21 = x22 - x1, y21 = y22 - y1, r21 = r2 - r1, l = Math.sqrt(x21 * x21 + y21 * y21);
  return {
...
function encloseBasis3(a4, b, c6) {
  var x1 = a4.x, y1 = a4.y, r1 = a4.r, x22 = b.x, y22 = b.y, r2 = b.r, x32 = c6.x, y32 = c6.y, r3 = c6.r, a22 = x1 - x22, a32 = x1 - x32, b22 = y1 - y22, b32 = y1 - y32, c22 = r2 - r1, c32 = r3 - r1, d1 = x1 * x1 + y1 * y1 - r1 * r1, d2 = d1 - x22 * x22 - y22 * y22 + r2 * r2, d3 = d1 - x32 * x32 - y32 * y32 + r3 * r3, ab4 = a32 * b22 - a22 * b32, xa = (b22 * d3 - b32 * d2) / (ab4 * 2) - x1, xb = (b32 * c22 - b22 * c32) / ab4, ya = (a32 * d2 - a22 * d3) / (ab4 * 2) - y1, yb = (a22 * c32 - a32 * c22) / ab4, A2 = xb * xb + yb * yb - 1, B3 = 2 * (r1 + xa * xb + ya * yb), C3 = xa * xa + ya * ya - r1 * r1, r = -(Math.abs(A2) > 1e-6 ? (B3 + Math.sqrt(B3 * B3 - 4 * A2 * C3)) / (2 * A2) : C3 / B3);
  return {
    x: x1 + xa + xb * r,
    y: y1 + ya + yb * r,
    r
...
function binary_default(parent, x0, y0, x1, y1) {
  ...
  function partition(i2, j, value, x02, y02, x12, y12) {
    ...
    if (x12 - x02 > y12 - y02) {
      ...
      partition(i2, k2, valueLeft, x02, y02, xk, y12);
      partition(k2, j, valueRight, xk, y02, x12, y12);
    } else {
      var yk = value ? (y02 * valueRight + y12 * valueLeft) / value : y12;
      partition(i2, k2, valueLeft, x02, y02, x12, yk);
      partition(k2, j, valueRight, x02, yk, x12, y12);
    }
  }
}
...
function area_default3(polygon) {
...
function contains_default3(polygon, point6) {
  var n = polygon.length, p = polygon[n - 1], x4 = point6[0], y4 = point6[1], x0 = p[0], y0 = p[1], x1, y1, inside = false;
  for (var i = 0; i < n; ++i) {
    p = polygon[i], x1 = p[0], y1 = p[1];
    if (y1 > y4 !== y0 > y4 && x4 < (x0 - x1) * (y4 - y1) / (y0 - y1) + x1)
      inside = !inside;
    x0 = x1, y0 = y1;
  }
  return inside;
...
function radial() {
  ...
  scale2.range = function(_) {
    return arguments.length ? (squared.range((range3 = Array.from(_, number3)).map(square)), scale2) : range3.slice();
  };
  scale2.rangeRound = function(_) {
...
function transformer3() {
  var x0 = 0, x1 = 0.5, x22 = 1, s2 = 1, t03, t13, t22, k10, k21, interpolator = identity2, transform2, clamp = false, unknown;
  function scale2(x4) {
  ...
  scale2.domain = function(_) {
    return arguments.length ? ([x0, x1, x22] = _, t03 = transform2(x0 = +x0), t13 = transform2(x1 = +x1), t22 = transform2(x22 = +x22), k10 = t03 === t13 ? 0 : 0.5 / (t13 - t03), k21 = t13 === t22 ? 0 : 0.5 / (t22 - t13), s2 = t13 < t03 ? -1 : 1, scale2) : [x0, x1, x22];
  };
  scale2.clamp = function(_) {
    return arguments.length ? (clamp = !!_, scale2) : clamp;
  };
  ...
  return function(t) {
    transform2 = t, t03 = t(x0), t13 = t(x1), t22 = t(x22), k10 = t03 === t13 ? 0 : 0.5 / (t13 - t03), k21 = t13 === t22 ? 0 : 0.5 / (t22 - t13), s2 = t13 < t03 ? -1 : 1;
    return scale2;
  };
}
function diverging() {
...
function arc_default() {
  ...
  function arc() {
    var buffer, r, r0 = +innerRadius.apply(this, arguments), r1 = +outerRadius.apply(this, arguments), a0 = startAngle.apply(this, arguments) - halfPi3, a1 = endAngle.apply(this, arguments) - halfPi3, da2 = abs3(a1 - a0), cw = a1 > a0;
    if (!context)
    ...
    if (!(r1 > epsilon7))
      ...
      var a01 = a0, a11 = a1, a00 = a0, a10 = a1, da0 = da2, da1 = da2, ap = padAngle.apply(this, arguments) / 2, rp = ap > epsilon7 && (padRadius ? +padRadius.apply(this, arguments) : sqrt2(r0 * r0 + r1 * r1)), rc = min3(abs3(r1 - r0) / 2, +cornerRadius.apply(this, arguments)), rc0 = rc, rc1 = rc, t03, t13;
      if (rp > epsilon7) {
        var p0 = asin(rp / r0 * sin2(ap)), p1 = asin(rp / r1 * sin2(ap));
        if ((da0 -= p0 * 2) > epsilon7)
      ...
      if (rc > epsilon7) {
        ...
        if (da2 < pi4) {
          if (oc = intersect(x01, y01, x00, y00, x11, y11, x10, y10)) {
            var ax = x01 - oc[0], ay = y01 - oc[1], bx = x11 - oc[0], by = y11 - oc[1], kc = 1 / sin2(acos((ax * bx + ay * by) / (sqrt2(ax * ax + ay * ay) * sqrt2(bx * bx + by * by))) / 2), lc = sqrt2(oc[0] * oc[0] + oc[1] * oc[1]);
...
function area_default4(x0, y0, y1) {
  var x1 = null, defined = constant_default9(true), context = null, curve = linear_default, output = null, path2 = withPath(area);
...
function pie_default() {
  var value = identity_default4, sortValues = descending_default, sort2 = null, startAngle = constant_default9(0), endAngle = constant_default9(tau5), padAngle = constant_default9(0);
  function pie(data) {
...
function areaRadial_default() {
  var a4 = area_default4().curve(curveRadialLinear), c6 = a4.curve, x0 = a4.lineX0, x1 = a4.lineX1, y0 = a4.lineY0, y1 = a4.lineY1;
...
function link(curve) {
  let source = linkSource, target = linkTarget, x4 = x3, y4 = y3, context = null, output = null, path2 = withPath(link2);
  function link2() {
    let buffer;
  ...
  link2.x = function(_) {
    return arguments.length ? (x4 = typeof _ === "function" ? _ : constant_default9(+_), link2) : x4;
  };
  link2.y = function(_) {
    return arguments.length ? (y4 = typeof _ === "function" ? _ : constant_default9(+_), link2) : y4;
  };
...
var kr = sin2(pi4 / 10) / sin2(7 * pi4 / 10);
var kx = sin2(tau5 / 10) * kr;
var ky = -cos2(tau5 / 10) * kr;
var star_default = {
  draw(context, size) {
...
var wye_default = {
  draw(context, size) {
    ...
    context.lineTo(c5 * x0 - s * y0, s * x0 + c5 * y0);
    context.lineTo(c5 * x1 - s * y1, s * x1 + c5 * y1);
    context.lineTo(c5 * x22 - s * y22, s * x22 + c5 * y22);
    context.lineTo(c5 * x0 + s * y0, c5 * y0 - s * x0);
    context.lineTo(c5 * x1 + s * y1, c5 * y1 - s * x1);
